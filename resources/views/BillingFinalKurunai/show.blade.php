...
<h3 class="text-center">Showing <span class="text-danger">Bill Number {{ $finaldatas->bill_number }} </span>, Dated <span class="text-danger">{{ $finaldatas->sale_date->format('d-m-Y') }}</span></h3>
<p class="lead text-center">See All. <a href="{{ route('BillingFinalKurunai.index') }}"> Go Back?</a></p>
<hr>

@include('partials.alerts.errors')

{!! Form::open([
    'route' => 'BillingKurunai.store', 'id' => 'formidda'
]) !!}
        ...
        @foreach($items as $item)
            <tr>
               ...
               <!-- <td class="text-center hidden-print">
                    {!! Form::open([
                       'method' => 'DELETE',
                       'route' => ['BillingKurunai.destroy', $item->id]
                   ]) !!}
...
<div class="clearfix"></div>
<div class="col-md-1 col-md-offset-11"><input value="{{ $finaldatas->id }}" class="hidden" id="bill_print_id"/> <button type="button" class="btn btn-danger" id="bill_print_kurunai">Print</button></div>
@stop