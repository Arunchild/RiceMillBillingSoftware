...
class ReportsKurunaiController extends Controller {
	...
	public function index()
	{
		...
		$totals = DB::table('billing_final_kurunais')
				->select(DB::raw('SUM(total_kg) as total_kg'), DB::raw('SUM(grand_total) as grand_total'), DB::raw('SUM(discount) as discount'))
				->Where($matchThese)
				->whereNull('deleted_at')
				->get();

		$items = BillingFinalKurunai::withTrashed()->Where($matchThese)->orderby('id', 'desc')->Paginate(1000);



		$products = DB::table('billing_kurunais')
		...
		return view('ReportsKurunai.index', compact('items', 'totals', 'products', 'today'));
	}
	...
	public function reportbydate(Request $request)
	{
		...
		$items = BillingFinalKurunai::withTrashed()
				->whereBetween('sale_date',array($from_date,$to_date))
				->orderby('id', 'desc')
				->Paginate(100000);
		...
		return view('ReportsKurunai.reportdata', compact('items', 'totals', 'products'));
	}
	...
	public function reportbydateprint(Request $request)
	{
		...
		return view('ReportsKurunai.reportdataprint', compact('items', 'totals', 'products', 'from_date', 'to_date', 'settings', 'company'));
	}
	...
	public function ShowReports(Request $request)
	{
		...
		foreach ($groupmasters as $row) {
			...
			$result = DB::select('SELECT sum(quantity) as quantity, sum(total) as total, group_name, product_name, selling_price, measuring_type FROM billings WHERE sale_date BETWEEN "' . $from_date . '" AND "' . $to_date . '" AND group_name = "' . $group_name . '" GROUP BY product_code ');
	...
	public function DownloadReports(Request $request)
	{
		...
		foreach ($groupmasters as $row) {
			...
			${'result' . $i} =  DB::select('SELECT sum(quantity) as quantity, sum(total) as total, group_name, product_name, selling_price, measuring_type FROM billings WHERE sale_date BETWEEN "' . $from_date . '" AND "' . $to_date . '" AND group_name = "' . ${'group_name' . $i} . '" GROUP BY product_code ');
		...
		Excel::create('SalesReport', function($excel) use($items, $result0, $result1, $result2, $group_name0, $group_name1, $group_name2, $tax_percentage0, $tax_percentage1, $tax_percentage2, $from_date, $to_date ) {
			...
			$excel->sheet('SalesReport', function ($sheet) use($items, $result0, $result1, $result2, $group_name0, $group_name1, $group_name2, $tax_percentage0, $tax_percentage1, $tax_percentage2, $from_date, $to_date) {
				...
				$sheet->loadView('ReportsKurunai.download',  compact('result0', 'result1', 'result2', 'items', 'group_name0', 'group_name1', 'group_name2', 'tax_percentage0', 'tax_percentage1', 'tax_percentage2', 'from_date', 'to_date'));
