...
class BillingFinalOthersController extends Controller {
	...
	public function index()
	{
		...
		$items = BillingFinalOthers::withTrashed()->orderby('id', 'desc')->Paginate(1000);

		return view('BillingFinalOthers.index', compact('items', 'totals'));
	}
	...
	public function store(Request $request)
	{
		...
		BillingFinalOthers::create($input);
		...
		$final = BillingFinalOthers::Where($matchThese)->first();
		$products = BillingOthers::Where($matchThese)->get();
		...
		Session::flash('flash_message', 'Billing Others successfully added!');
	...
	{
		// get the nerd
		$finaldatas = BillingFinalOthers::find($id);
	...
	/*  for($c=0; $c< $settings->copy; $c++){
		...
		return view('BillingFinalOthers.show', compact('items', 'sum', 'finaldatas'));
	}


	public function bill_print_others(Request $request)
	{
		$input = $request->all();
		...
		for($c=0; $c< $settings->copy; $c++){
			...
			if($handle)
			{
                ...
                if($discount > 0){
                    ...
                    printer_write($handle, "\n");
                    printer_write($handle, str_pad("NET Total :   ", 27, " ", STR_PAD_LEFT)."  ".str_pad(" ", 3, " ", STR_PAD_BOTH).str_pad($net_amount, 8, " ", STR_PAD_LEFT));
                    printer_write($handle, "\n");
	...
	{
		$items = BillingFinalOthers::findOrFail($id);
		return view('BillingFinalOthers.edit', compact('items'));
	}
	...
	public function update($id, Request $request)
	{
		...
		return redirect()->route('BillingFinalOthers.index');
	}
	...
	{
		$item = BillingFinalOthers::findOrFail($id);
		...
		foreach($products as $product){
			$id = $product->id;
			$deletethisproduct = BillingOthers::findOrFail($id);
			$deletethisproduct->forceDelete();
		}

		Session::flash('flash_message', 'Bill Others successfully deleted!');
