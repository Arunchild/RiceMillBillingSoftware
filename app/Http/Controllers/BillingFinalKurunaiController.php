...
class BillingFinalKurunaiController extends Controller {
	...
	public function index()
	{
		...
		$items = BillingFinalKurunai::withTrashed()->orderby('id', 'desc')->Paginate(1000);

		return view('BillingFinalKurunai.index', compact('items', 'totals'));
	}
	...
	public function store(Request $request)
	{
		...
		BillingFinalKurunai::create($input);
		...
		$final = BillingFinalKurunai::Where($matchThese)->first();
		$products = BillingKurunai::Where($matchThese)->get();
		...
		for($c=0; $c< $settings->copy; $c++){
			...
			if($handle)
			{
				printer_start_doc($handle, "BillingKurunai");
	...
	{
		// get the nerd
		$finaldatas = BillingFinalKurunai::find($id);
		...
		return view('BillingFinalKurunai.show', compact('items', 'sum', 'finaldatas'));
	}

	public function bill_print_kurunai(Request $request)
	{
		$input = $request->all();
		...
		//return view('BillingKurunaiFinal.show', compact('items', 'sum', 'finaldatas'));
	}
	...
	{
		$items = BillingFinalKurunai::findOrFail($id);
		return view('BillingFinalKurunai.edit', compact('items'));
	}
	...
	public function update($id, Request $request)
	{
		...
		return redirect()->route('BillingFinalKurunai.index');
	}
	...
	{
		$item = BillingFinalKurunai::findOrFail($id);
		...
		foreach($products as $product){
			$id = $product->id;
			$deletethisproduct = BillingKurunai::findOrFail($id);
