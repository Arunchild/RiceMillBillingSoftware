...
class ReportsController extends Controller
{
	...
	public function index()
	{
		...
		$items = BillingFinal::withTrashed()->Where($matchThese)->orderby('id', 'desc')->Paginate(1000);



		$products = DB::table('billings')
		...
		return view('Reports.index', compact('items', 'totals', 'products', 'today'));
	}
	...
	public function reportbydate(Request $request)
	{
		...
		$items = BillingFinal::withTrashed()
					->whereBetween('sale_date',array($from_date,$to_date))
					->orderby('id', 'desc')
					->Paginate(100000);
		...
		return view('Reports.reportdata', compact('items', 'totals', 'products'));
	}
	...
	public function reportbydateprint(Request $request)
	{
		...
		return view('Reports.reportdataprint', compact('items', 'totals', 'products', 'from_date', 'to_date', 'settings', 'company'));
	}
	...
	public function DownloadReports(Request $request)
	{
		...
		Excel::create('SalesReport', function($excel) use($items, $result0, $result1, $result2, $group_name0, $group_name1, $group_name2, $tax_percentage0, $tax_percentage1, $tax_percentage2, $from_date, $to_date ) {
			...
			$excel->sheet('SalesReport', function ($sheet) use($items, $result0, $result1, $result2, $group_name0, $group_name1, $group_name2, $tax_percentage0, $tax_percentage1, $tax_percentage2, $from_date, $to_date) {
				...
				$sheet->loadView('Reports.download',  compact('result0', 'result1', 'result2', 'items', 'group_name0', 'group_name1', 'group_name2', 'tax_percentage0', 'tax_percentage1', 'tax_percentage2', 'from_date', 'to_date'));
